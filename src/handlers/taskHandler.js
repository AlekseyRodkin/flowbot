...
class TaskHandler {
  ...
  async showTodayTasks(ctx, taskService, user, editMessage = false) {
    try {
      ...
      // Level увеличивается при ЗАВЕРШЕНИИ всех задач дня.
    ...
    try {
      ...
      setImmediate(async () => {
        ...
        if (completedTasks === totalTasks && totalTasks > 0) {
          ...
          // 2. ПОТОМ увеличиваем уровень пользователя при завершении всех задач дня
          // ✅ ПРАВИЛЬНАЯ ЛОГИКА: Level увеличивается при ЗАВЕРШЕНИИ дня, не при отправке задач
          // Это позволяет пользователю повторить день если не завершил задачи
