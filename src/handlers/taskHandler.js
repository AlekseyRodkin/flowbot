...
class TaskHandler {
  ...
  async completeTask(ctx, taskService, taskId) {
    try {
      ...
      // 2. СРАЗУ отправить ответ пользователю (не ждём статистику)
      await ctx.answerCbQuery('✅ Отлично!');

      // 3. Параллельно: получить статистику + обновить UI
  ...
  async updateTaskMessage(ctx, taskService, userId) {
    try {
      ...
      const userLevel = tasks[0]?.user?.level || 1;
      const hardTasksCount = tasksByType.hard.length;
...
*ТЫ ${g(user, 'ВЫПОЛНИЛ', 'ВЫПОЛНИЛА')} ВСЕ 30 ЗАДАЧ!*
...
⚡ Productivity: ${stats.productivity_index || 100}%
