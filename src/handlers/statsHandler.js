...
class StatsHandler {
  ...
  async showStats(ctx, user, userService, taskService) {
    try {
      ...
      // Fallback: если нет статистики, используем текущий level
      const currentDay = stats.totalDays || (user.level || 1);
      if (currentDay <= 30) {
  ...
  async generateReport(ctx, user) {
    try {
      ...
      // Текущий день программы = текущий level
      const reportDay = user.level || 1;

      report += `👤 *Пользователь:* ${user.first_name}\n`;
      if (reportDay <= 30) {
        report += `📅 *День программы:* ${reportDay}/30\n`;
      } else {
        report += `📅 *День программы:* ${reportDay} (завершена! 🎉)\n`;
      }
